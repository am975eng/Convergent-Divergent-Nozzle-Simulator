...
def gen_MOC_FLN(M_exit, r_throat, k=1.4, div=7, print_flag=False, plot_flag=False):
    ...
    custom_cmap = LinearSegmentedColormap.from_list('red_green', colors, N=n_colors)

    for row_num, row_len in enumerate(grid_row_pts):
        for row_index in range(row_len):                        # Iterate through each row
            if idx <= div:                                      # Expansion section wall points
                ...
                if plot_flag:
                    plt.plot([x_n[idx-1], x_n[idx]], [y_n[idx-1], y_n[idx]], "k-", linewidth=1)
                ...
                slope = (theta_n[idx-offset] - mu_n[idx-offset] - mu_n[idx])/2
                x_n[idx] = - (y_n[idx-offset] / np.tan(slope)) + x_n[idx-offset]
                y_n[idx] = 0

                if plot_flag:
                    plt.plot([x_n[idx-offset], x_n[idx]], [y_n[idx-offset], y_n[idx]], c=custom_cmap(int(col[idx])), linewidth=1)
                    plt.scatter(x_n[idx], y_n[idx], s=1)
                    plt.annotate(str(idx), (x_n[idx],y_n[idx]))
                ...
                if plot_flag:
                    plt.plot([x_n[idx-1], x_n[idx]], [y_n[idx-1], y_n[idx]], c=custom_cmap(int(col[row_num])), linewidth=1)
                    plt.plot([x_n[idx-offset], x_n[idx]], [y_n[idx-offset], y_n[idx]], "k-", linewidth=1)
                    plt.scatter(x_n[idx], y_n[idx], s=1)
                    plt.annotate(str(idx), (x_n[idx],y_n[idx]))
    ...
    if print_flag:
        for i in range(len(theta_n)):
            print(f" i {i} theta_n {theta_n[i]*180/np.pi:.3f} nu_n {nu_n[i]*180/np.pi:.3f} K- {K_minus[i]*180/np.pi:.3f} K+ {K_plus[i]*180/np.pi:.3f} M_n {M_n[i]:.3f} mu_n {mu_n[i]*180/np.pi:.3f} x_n {x_n[i]:.3f} y_n {y_n[i]:.3f} col {col[i]}")
