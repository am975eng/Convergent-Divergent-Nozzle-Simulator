...
def gen_MOC_MLN(
    M_exit, r_throat, k=1.4, div=7, print_flag=False, plot_flag=False
):
    """
    ...
    theta_wall_max = (
        AT.calc_prandtl_meyer(M_exit, k) / 2
    ...
    if print_flag:
        for i in range(len(theta_n)):
            print(
                f"i {str(i)} K- {K_minus[i]*180/np.pi:.3f}"
    ...
    popt, pcov = curve_fit(
        high_poly_law, x_contour, y_contour, p0=p0, maxfev=10000
    )
    a, b, c, d, e = popt
...
def gen_MOC_FLN(
    ...
    for row_num, row_len in enumerate(grid_row_pts):  # Loop row lengths
        for row_index in range(row_len):  # Loop each row
            if idx <= div:  # Expansion section
                ...
                M_n[idx] = root_scalar(
                    AT.RS_mach_prandtl_meyer,  # Mach No.
                    bracket=[1, 10],
                    args=(nu_n[idx], k),
                ).root
    ...
    if print_flag:
        for i in range(len(theta_n)):
            print(
                f" i {i} theta_n {theta_n[i]*180/np.pi:.3f}"
...
def gen_rao_bell(
    r_throat, r_outlet, k=1.4, len_per=80, print_flag=False, plot_flag=False
):
    ...
    theta_para_n, theta_para_e = AT.calc_rao_para_ang(
        area_ratio_outlet, len_per
    )
    ...
    N_y = (
        0.382 * r_throat * np.sin(theta_para_n - np.pi / 2)
        + 0.382 * r_throat
        + r_throat
    )
    E_x = (
        (len_per / 100)
        * ((area_ratio_outlet**0.5) - 1)
        * r_throat
        / np.tan(15 * np.pi / 180)
    )
...
if __name__ == "__main__":
    # Test script
    x_contour, y_contour, x_n, y_n, M_n = gen_MOC_MLN(
    ...
    x_contour, y_contour, x_n, y_n, M_n = gen_MOC_FLN(
